...
export class AlertBannerComponent {
  ...
  readonly icon = computed(() => (this.tone === 'success' ? 'lucideCheck' : this.tone === 'error' ? 'lucideCircleAlert' : 'lucideInfo'));

  readonly badgeClasses = computed(() => {
    if (this.variant === 'toast') return 'bg-success-text';
    return this.tone === 'success' ? 'bg-success-text' : this.tone === 'error' ? 'bg-error' : 'bg-brand';
  });
